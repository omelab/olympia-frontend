...
const FeaturesAction = ({ record, isTrash }: any) => {
  const [moveModal, setMoveModal] = useState(false);
  const [, setKeyword] = useState('');

  const [restoreId] = useRestorefeaturesMutation();
  ...
  const retoreHandler = async (id: any) => {
    try {
      ...
      if (!res?.error) {
        message.success('Features restore successfully.');
      } else {
  ...
  const [deleteItem] = useDeletefeaturesMutation();
        ...
        {!isTrash
          ? (
              <Tooltip placement="top" title="Update">
                <Link href={`/admin/features/editFeatures/${record?.id}`}>
